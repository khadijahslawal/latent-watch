"""E1: Answer-only baseline training.

Trains a LoRA adapter on Llama-3.2-1B to predict HIGH_RISK / LOW_RISK
from the prompt alone, with no reasoning trace.

Entry points:
  - Called from notebooks/04_train_pilot.ipynb (Colab)
  - Called from scripts/train_answer_only.sh (A100)
"""
...
from latent_watch.training.formatters import format_answer_only
...
def _resolve_hparams(hparams: dict | None, config_path: str | None) -> dict:
    """Merge config yaml (if provided) with inline hparams. Inline wins."""
    base = {}
...
def train_answer_only(
    dataset_dir: str | Path,
    output_dir: str | Path,
    hparams: dict | None = None,
    config_path: str | None = None,
) -> Path:
    """Train the answer-only baseline (E1).

    Args:
        dataset_dir: Path to answer_only/ split directory containing
                     train.jsonl, validation.jsonl, test.jsonl.
        output_dir: Root checkpoint directory (e.g. checkpoints/answer_only).
        hparams: Inline hyperparameter dict. Overrides config_path values.
        config_path: Optional path to configs/training/llama_1b_answer_only.yaml.
    ...
    train_ds = PromptRiskDataset(
        train_rows, tokenizer, format_answer_only, max_seq_length=max_seq_len
    )
    val_ds = PromptRiskDataset(
        val_rows, tokenizer, format_answer_only, max_seq_length=max_seq_len
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E1: Answer-only training")
    parser.add_argument("--dataset-dir", required=True)
    parser.add_argument("--output-dir", default="checkpoints/answer_only")
